import React from 'react';

export const fr = {
  navbar: {
    whatWeDo: "Ce que nous faisons",
    products: "Nos Produits",
    projects: "Projets",
    news: "Actualités",
    careers: "Carrières",
    contact: "CONTACTEZ-NOUS"
  },
  hero: {
    slides: [
      {
        id: 1,
        image: "https://images.unsplash.com/photo-1541888946425-d81bb19240f5?q=80&w=2070&auto=format&fit=crop",
        title: "82% RÉDUCTION CO2",
        subtitle: "Technologie de stabilisation des sols durable pour un avenir plus vert."
      },
      {
        id: 2,
        image: "/images/hero2.png",
        title: "RENTABLE",
        subtitle: "Réduisez les coûts de construction tout en améliorant la durabilité et la longévité."
      },
      {
        id: 3,
        image: "/images/hero3.png",
        title: "IMPACT MONDIAL",
        subtitle: "Technologie éprouvée appliquée dans des projets d'infrastructure dans le monde entier."
      }
    ],
    cta: "Ce que nous faisons"
  },
  about: {
    title: ["Nous sommes PowerCem", "Italie"],
    p1: "PowerCem Technologies est un leader mondial dans le domaine de la modification in situ des matériaux et de la stabilisation des sols. Nous fournissons des solutions durables et innovantes pour les secteurs de la construction et des infrastructures.",
    p2: "Notre mission est de réduire l'empreinte carbone de la construction tout en améliorant la qualité et la longévité des routes et des structures. Avec notre branche italienne, nous apportons cette expertise au cœur de la Méditerranée."
  },
  products: {
    items: [
      { name: "RoadCem", image: "/images/ok4.png", link: "#" },
      { name: "ImmoCem", image: "/images/ok2.png", link: "#" },
      { name: "ConcreCem", image: "/images/ok3.png", link: "#" }
    ],
    cta: "EN SAVOIR PLUS"
  },
  soil: {
    title: ["Stabilisation des sols pour", "un avenir durable"],
    cta: "EN SAVOIR PLUS"
  },
  customers: {
    title: ["Nous nous engageons à fournir les solutions", "de stabilisation des sols les plus durables à tous nos", "clients"],
    stats: [
      { value: "4.500.000", unit: "M²", description: "Équivalent à l'absorption de CO2 de 4,5 millions de m² de forêt" },
      { value: "24.000.000", unit: "M²", description: "Stabilisés et renforcés pour un avenir durable" },
      { value: "1.350.000.000", unit: "KG", description: "Réduction de CO2 : Renforcer la résilience de notre planète" }
    ]
  },
  collaboration: {
    title: "Collaboration mondiale, impact local",
    p1: "PowerCem Technologies collabore mondialement avec un réseau de partenaires de confiance. Ces représentants locaux jouent un rôle crucial dans le soutien de nos solutions innovantes sur leurs marchés.",
    p2: "Nous accueillons de nouveaux partenariats avec des organisations qui partagent notre engagement envers l'innovation et la durabilité, nous aidant à étendre l'impact de nos produits dans le monde entier.",
    cta: "LIRE PLUS",
    contactCard: {
      title: "Contactez-nous",
      subtitle: "Pays",
      button: "Contactez-nous"
    }
  },
  map: {
    continents: {
      na: "Amérique du Nord",
      sa: "Amérique du Sud",
      eu: "Europe - Siège PowerCem",
      af: "Afrique",
      as: "Asie",
      au: "Australie"
    }
  },
  footer: {
    company: { title: "Entreprise", links: ["À propos", "Projets", "Actualités", "Contact"] },
    products: { title: "Nos produits", links: ["RoadCem", "ImmoCem", "ConcreCem"] },
    contact: { title: "Contact", address: "Via Roma 123, 00184 Roma, Italia", email: "info@powercem.it", phone: "+39 06 1234 5678" },
    follow: "Suivez-nous",
    copyright: "© 2026 PowerCem Technologies Italie"
  },
  trusted: {
    title: "Des partenariats de confiance pour l'innovation des infrastructures mondiales"
  }
};