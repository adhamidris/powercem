  ...
  return (
    <nav
      className={`fixed z-50 transition-all duration-300 flex items-center justify-between px-6 md:px-10 h-20 rounded-full border left-1/2 -translate-x-1/2 ${
        isScrolled
          ? 'top-2 w-[98%] max-w-[1920px] bg-white shadow-lg border-gray-100'
          : 'top-6 w-[98%] max-w-[1920px] bg-black/20 backdrop-blur-md border-white/10'
      }`}
    >
      <div className="w-full flex items-center justify-between h-full">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2">
           <div className={`text-2xl font-bold tracking-tighter transition-colors ${isScrolled ? 'text-power-blue' : 'text-white'}`}>
             POWER<span className={isScrolled ? 'text-power-dark' : 'text-white'}>CEM</span>
           </div>
        </Link>

        {/* Desktop Navigation */}
        <div className={`hidden lg:flex items-center gap-8 font-medium text-[15px] transition-colors ${isScrolled ? 'text-power-dark' : 'text-white'}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <button className={`lg:hidden p-2 ${isScrolled ? 'text-power-dark' : 'text-white'}`}>
